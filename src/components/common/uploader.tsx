...
export default function Uploader({
  ...
  const handleDelete = (image: string, id: any) => {
    console.log('image', image, files, id);
  ...
  const handleFeatured = (image: string, id: any, value: boolean) => {
    const images = files.map((file) => {
      // if there is a file with featured true, then set it to false
      if (value && file.id !== id) {
        file.featured = false;
      }
      if (file.id === id) {
        ...
        >
          ...
          {multiple ? (<>
            <button
              ...
              onClick={() => handleDelete(file.thumbnail, file.id)}
            >
            ...
            <input
              ...
              onChange={(e) => handleFeatured(file.thumbnail, file.id, e.target.checked)}
