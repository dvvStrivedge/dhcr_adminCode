...
> & {
  ...
  related_products: Pick<Product, 'id' | 'name'>[];
...
export function getProductInputValues(
  ...
  return {
    ...
    related_products: related_products.map((product) => product?.id),
    // tags: tags.map((tag) => tag?.id),
    includes: values?.includes,
    pickup_locations: values?.pickup_locations,
    image: omitTypename<any>(image),
    gallery: values.gallery?.map((gi: any) => omitTypename(gi)),
    quantity,
    three_days_discount: values?.three_days_discount,
    one_week_discount: values?.one_week_discount,
    one_month_discount: values?.one_months_discount,
    // digital_file: {
